...
export function Footer({ chordPrefs }: FooterProps) {
  return (
    <div className="text-[10px] text-text-faint leading-[1.8] border-t border-border-faint pt-2.5">
      ...
      コードトーン強調で {formatQuality('maj7', chordPrefs)} / {formatQuality('7', chordPrefs)} / {formatQuality('m7', chordPrefs)} / {formatQuality('m7♭5', chordPrefs)} / {formatQuality('mMaj7', chordPrefs)} / {formatQuality('aug', chordPrefs)} を比較可能。
    </div>
  );
}

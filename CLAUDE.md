...
│   ├── configLoader.ts              — 汎用 JSON コンフィグローダー factory (deepMerge, createConfigLoader) + 全設定型定義 (DrumConfig/CompConfig/BassConfig(kitGains/customWAV(detune/decayTime/volume/releaseFadeMs)/prng/defaultDuration/velocityHumanize/tripletGrace/patterns.swing(SwingBassParams)/altOctaveProb/styleOverrides(SwingStyleOverrides: altOctaveProb))/PianoConfig/AudioConfig)
...
│       └── configLoader.test.ts   — 14 tests (deepMerge, 各getXxxConfig未ロードデフォルト値, releaseFadeMs)
├── hooks/
...
- **ベース設定 JSON** (`public/bass-config.json`): `kitGains` (キット別ゲイン倍率)、`customWAV` (detune/decayTime/volume/releaseFadeMs)、`prng` (シード)、`defaultDuration` (デフォルト0.86拍)、`velocityHumanize` (base/range)、`tripletGrace` (probability/velocity/beatOffset)、`patterns.swing` (SwingBassParams: テンプレート・コンター・アプローチ重み) — ウォーキングベース生成+再生チューニング

...
│   ├── phraseGenerator.ts           — generatePhrase(), buildNotePool(), getApproachNotes()
│   └── __tests__/
...
│       ├── phraseGenerator.test.ts  — 53 tests (構造不変条件、ビバップ統計検証)
│       └── audioEngine.test.ts      — 10 tests (Karplus-Strong, コードストラム)
└── components/
    ├── Fretboard/                   — SVG指板描画 (Fretboard, FretboardNote, GhostNote, PhrasePath)
    ├── Controls/                    — RootSelector, ModeSelector, PositionSelector, OptionBar, VoicingGrid, PhraseControls
    ├── Footer.tsx
...
4. 開始音 (beat 1) 決定: CT からランダム選択 (ゴールと12半音以内)
5. 逐次生成 (beats 2-7): 候補スコアリング (インターバル分布, 輪郭一致, 方向転換, ゴール近接)
...
**描画**: PhrasePath.tsx — Catmull-Rom → Cubic Bezier 変換, 色 #FF6B9D, 矢印マーカー, ノート形状 (CT=塗り円, アプローチ=ダイヤ, スケール=枠円), 拍番号

**UI**: OptionBar「フレーズ」チェックボックス → PhraseControls (ソース/アプローチ/Generate/◀▶履歴)
有効条件: `selPosIds.length === 1 && !overlay && !is8Note`

**テスト**: 53テスト (構造不変条件 + N=100 ビバップ統計検証: Parker分析データとのインターバル分布/方向転換/音域比較)
...
- フレーズジェネレーター: ビバップライン自動生成、Catmull-Rom SVG曲線表示、アプローチパターン、統計検証済

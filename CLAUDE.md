...
│       ├── phraseGenerator.test.ts  — 70 tests (構造不変条件、ビバップ統計検証、メタデータ記録、startHint連結、骨格近接、ゴールVL保持)
│       ├── phraseQualityAudit.test.ts — 409 tests (17条件×23品質アサーション + 3進行コンテキスト)
...
5. **デジタルパターン試行**: beat 1 確定後 + 強拍でパーカー語彙パターンを42%確率で一括コミット (1フレーズ1回)
...
8. ポスト処理: 同音反復修正、beat 8 距離ガード (>7st でVL互換CT優先フォールバック)
...
**スコアリング重み**:
- 順次進行 (1-2半音): 60, 3度 (3-4半音): 18, 4度 (5半音): 14, 5度+: 5
...
**テスト**: 70テスト (構造不変条件 + N=100 ビバップ統計検証 + メタデータ記録 + startHint連結 + 骨格近接 + ゴールVL保持) + 409テスト (品質監査: 17条件×23アサーション + 3進行コンテキスト) + 33テスト (分析・メタデータ・ナラティブ)
...
- フレーズジェネレーター: ビバップライン自動生成 (ハーモニック・スケルトン+デジタルパターン33個+ガイドトーン階層+エクステンション音+ビバップスケール+コード間VL+モチーフ記憶)、SVG曲線表示、アプローチパターン、品質監査・統計検証済、単体再生 (▶ Play)
- フレーズ分析: analyzePhrase(), PhraseAnalysisPanel (折りたたみUI、度数/インターバル/機能ラベル、生成メタデータ可視化、ナラティブ)

...
│   ├── bassPatterns.ts              — generateBassLine(), playSmplrBassLine(), BassSamplerSet, loadBassSampler(), getBassSampler(), BassPatternDB, loadBassPatternDB(), getBassPatternDB() — スタイル別ベースライン生成 (Swing/Bossa/Ballad/Latin) + smplr acoustic_bass / カスタム WAV 再生 (voice stealing, letRing)
...
│   ├── configLoader.ts              — 汎用 JSON コンフィグローダー factory (deepMerge, createConfigLoader) + 全設定型定義 (DrumConfig/CompConfig/BassConfig(kitGains/customWAV)/PianoConfig/AudioConfig)
...
│       ├── bassPatterns.test.ts     — 20 tests (ベースライン生成、音域検証、拍数別、アプローチノート、スタイル別、カスタムWAV設定、BassPatternDB)
...
├── data/                                — マスターデータ (Git LFS: export MIDI, drum-kits.json)
└── output/                              — 再生成可能な全出力 (gitignored)
...
- **ウォーキングベース** (smplr): acoustic_bass SoundFont / カスタム WAV でコード進行に合わせたベースラインを自動生成。`generateBassLine()` がコード品質・拍数・次コードルートからライン生成 (1拍=ルート、2拍=ルート+アプローチ、3-4拍=ルート→3rd/5th→5th/8va→半音アプローチ)。ミキサーにベースチャンネル (音量+ミュート) 追加。カスタム WAV: `public/bass/{kitName}/{noteName}_v{velocity}.wav`、voice stealing (単音楽器→前ノート停止)、letRing (コード遷移時に自然減衰)、WAV なしなら SoundFont にフォールバック
...
- マスターデータ: `scripts/data/export_licks.mid` (Cubase一括書出し、トラック名=ファイル名規約)
- マルチトラックモード: `python scripts/parse_licks.py scripts/data/export_licks.mid` (2トラック以上で自動判定)
- 個別ファイルモード: `python scripts/parse_licks.py scripts/output/midi/licks/` (従来互換)
- `split_midi.py` は不要 (パーサーが直接マルチトラックを処理)
...
- **ベース設定 JSON** (`public/bass-config.json`): `kitGains` (キット別ゲイン倍率)、`customWAV` (detune/decayTime/volume) — チューニング設定のみ

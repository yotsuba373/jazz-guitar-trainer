...
├── types/
│   └── music.ts                     — ChordSlot (lickIiVType フィールド含む), Progression, ChartMeasure, ChartLayout, ModeTemplate, PoolNote 等
├── constants/
...
│   ├── lickEngine.ts                — absolutePitch(), buildNotePool(), loadLickDB(), transposeLick(), mapLickToFretboard(), lickToGeneratedPhrase(), inferModeFromLick(), inferModeCandidates(), findBestPositionForLick(), selectBestInstance(), buildLickContext(), detectIiVPattern(), isIiVLickId(), buildIiVLickContext()
...
├── data/                                — ダウンロードデータ + リックDB + export.mid (gitignored)
└── output/
...
- **8va**: 同一インスタンス内でオクターブ切替 (デフォルト=低、ON=高)。viable シフト (≥50%カバレッジ、上位2件) が2つ以上ないと disabled
- **Hi**: ハイポジションインスタンスに切替。インスタンスが1つなら disabled

### フレーズ描画 & 分析

**描画**: PhrasePath.tsx — Catmull-Rom → Cubic Bezier 変換, per-beatグラデーション(#FFA0B0→#BBA0FF), ノート形状 (CT=塗り円, アプローチ=ダイヤ, スケール=枠円), 拍番号, 視認性改善。ii-V-long リックではコード品質変化ビート (`chordChangeBeat`) でライン分割+Vセクション先頭に強調リング。リック表示に使用。
...
- コード進行: 作成・編集・保存 (localStorage)、近接ポジション提案、キーボードナビ、コードインライン編集、chartLayout差分更新保持、「+ 小節」→ビートグリッド挿入フロー (追加ボタン廃止)、空小節 × 削除、選択ビートハイライト
...
- **ii-V リック対応**: `detectIiVPattern()` で連続コード (m7→7) の ii-V パターンを検出。ii コード選択時に ii-V タイプのリック (`maj-ii-v-short`, `maj-ii-v-long`, `min-ii-v-short`) を表示。`buildIiVLickContext()` で2コード分の指板コンテキストを生成。`ChordSlot.lickIiVType` に選択タイプを永続化。`isIiVLickId()` でリックIDが ii-V 由来かを判定
...
- `maj7` → `maj7` (C root)
- `m7♭5` → `m7b5` (D root)
- ii-V パターン検出: `detectIiVPattern(chords, idx)` — `chords[idx]` が m7 かつ `chords[idx+1]` が 7 の場合に `'major'|'minor'` を返す。LickPanel で ii コード選択時に ii-V タイプ (`maj-ii-v-short`, `maj-ii-v-long`, `min-ii-v-short`) のリックを追加表示。`buildIiVLickContext()` は ii+V の2コード分のプールを合成してフレーズを生成

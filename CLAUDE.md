...
├── types/
│   └── music.ts                     — ChordSlot (lickBeatOffset, lickAnacrusis フィールド含む), Progression (bpm, backingStyle, loopRange — 曲ごと保存), BackingStyle, ChartMeasure, ChartLayout, ModeTemplate, PoolNote 等
├── constants/
...
│   ├── configLoader.ts              — 汎用 JSON コンフィグローダー factory (deepMerge, createConfigLoader) + 全設定型定義 (DrumConfig/CompConfig/BassConfig(kitGains/customWAV(detune/decayTime/volume/octaveShift/releaseFadeMs(未使用)/legatoMaxInterval/legatoProbability)/prng/defaultDuration/velocityHumanize/tripletGrace/patterns.swing(SwingBassParams)/altOctaveProb/styleOverrides(SwingStyleOverrides: altOctaveProb))/PianoConfig/AudioConfig)
...
│   ├── swing.ts                     — swingBeatStart(), swingVolumeMult(), swingDurMult() — 全パラメータを getAudioConfig().swing から参照
│   └── __tests__/
...
│       └── configLoader.test.ts   — 16 tests (deepMerge, 各getXxxConfig未ロードデフォルト値, releaseFadeMs, legatoMaxInterval, legatoProbability)
├── hooks/
...
│   ├── useAutoPlay.ts               — 進行モード自動再生 (BPM, カウントイン, ループ, アナクルーシス)。ドラムスウィング量は getDrumConfig().drumSwingAmount から取得 (ハードコード廃止)
...
├── parse_licks.py                       — MIDI リックパーサー (マルチトラック/個別ファイル対応、120グリッド量子化、ルート移調、休符/三連符対応)
├── parse_drum_patterns.py               — MIDI ドラムパターンパーサー (8小節MIDI→JSON、ピッチベース、ロール不要)
├── parse_bass_phrases.py                — iReal Pro MusicXML+MIDI → スタイル別ベースフレーズ DB 生成パーサー (lyricist フィールドからスタイル自動分類、17曲対応)
├── compare_bass_by_style.ts             — スタイル別ベースライン比較スクリプト (iReal Pro vs 本システム)

├── data/                                — 設定ファイル (drum-kits.json)
└── output/                              — 全入出力 (gitignored: export MIDI, 中間ファイル, ダウンロードデータ)
    ├── midi/                            — split 済み個別 MIDI
    ├── omnibook/                        — ダウンロードした MusicXML
    ├── wjazzd.db                        — ダウンロードした WJD SQLite3
...
| スウィング | `swingEnabled` | 派生値 | — | — | `SWING_STYLES.has(backingStyle)` から自動判定 (Swing系=ON, Bossa/Ballad/Latin=OFF) |
| スウィング量 | `swingAmount` | `swingAmount` | 0.2 | — | 0-1 (デフォルト20%) — グローバル設定 (全曲共通) |
...
- **ウォーキングベース** (smplr): acoustic_bass SoundFont / カスタム WAV でコード進行に合わせたベースラインを自動生成。`generateBassLine()` がコード品質・拍数・次コードルート・スタイルからライン生成。**スタイル別パターンDB**: iReal Pro 17曲からスタイル別 (medium-swing/medium-up-swing/up-tempo-swing) に分離したパターンプール。フォールバックチェーン: up-tempo→medium-up→medium-swing→テンプレート。**スタイル別パラメータ** (bass-config.json の styleOverrides): Medium=デュレーション0.90/grace50%/半音アプローチ重視、Med-Up=デュレーション0.90/grace55%、Up-Tempo=デュレーション0.99(レガート)/grace5%/arpeggio40%。カスタム WAV: voice stealing + letRing、WAV なしなら SoundFont にフォールバック
...
- **バッキングスタイル** (6種): Medium Swing / Medium Up Swing / Up-Tempo Swing / Bossa / Ballad / Latin。スタイルに応じてコンピングリズム・ベースライン・ドラムパターンを一括切替。Swing系=Charlestonコンピング+4フィールウォーキングベース+スウィングライド (スタイル別パラメータ差異あり)、Bossa=シンコペーションコンピング+2フィールベース+クロススティック、Ballad=全音符コンピング+2フィールベース+ソフトライド、Latin=モントゥーノ風コンピング+トゥンバオベース+ストレート8thライド。`backingStyle` を localStorage 永続化、ミキサーで選択
...
```
public/licks.generated.json → loadLickDB() → LickDB
...
- **ポジション選択**: `findBestPositionForLick()` — インスタンス単位でリック音のピッチマッチ数を評価、最高スコアのポジションを選択。同スコア時は前ポジションとの近接度でタイブレイク
...
- 出力: `public/licks.generated.json` — `{ type: [{ notes, noteCount, beats, source?, anacrusis? }] }`
...
- マスターデータ: `scripts/output/export_licks.mid` (Cubase一括書出し、トラック名=ファイル名規約)
- マルチトラックモード: `python scripts/parse_licks.py scripts/output/export_licks.mid` (2トラック以上で自動判定)
- 個別ファイルモード: `python scripts/parse_licks.py scripts/output/midi/licks/` (従来互換)
...
DAWで打ち込んだ8小節MIDIドラムパターンをJSON DBに変換。ランタイムで8小節単位でランダム選択して再生。

- 入力: `scripts/output/midi/drums/{style}_{番号}.mid` (8小節=32拍, 4/4, 任意BPM)
- キットマッピング: `scripts/data/drum-kits.json` (スタイル → キットフォルダ名)
- 出力: `public/drum-patterns.generated.json`
- ピッチベース: MIDI ノート番号をそのまま保存 (ロール不要)
- 120グリッド量子化、ベロシティ保持 (0-127)、小節ごとに beatStart を 0-based に正規化
- 8小節ごとにパターンをランダム選択、小節内インデックスで該当小節を取得
- DB + カスタム WAV なしの場合はアルゴリズム生成にフォールバック
- パーサー実行時に全 pitch×velocity → WAV マッピングをレポート出力
- 複数スタイルが同一キット (WAV フォルダ) を共有可能
- 依存: `pip install pretty_midi`
...
- 配置: `public/drums/{kitName}/{noteName}_v{velocity}.wav` (例: `public/drums/jazz-kit/d#2_v60.wav`)
- キットフォルダ名は `drum-kits.json` で定義。複数スタイルが同じキットを共有可能
- ベロシティレイヤーは自由 (任意の数・任意の値)。再生時は最寄りの WAV を自動選択
- ノート名表記: C3=60 convention (Cubase 等標準)。パーサーがマッピングレポートを出力
...
- **ドラム設定 JSON** (`public/drum-config.json`): キットゲイン・サンプラー設定 (Hydrogen GM / カスタム WAV)・ベロシティレイヤー境界・パターン生成パラメータ (Swing/Bossa/Ballad/Latin) を外部化。ブラウザリロードだけで反映、パーサー再実行不要。ファイル不在・フィールド欠落時はデフォルト値にフォールバック

## カスタム WAV ベースサンプル

ドラムと同じ「カスタム WAV + voice stealing + letRing + 外部 config」パターンをベースにも適用。

- 配置: `public/bass/{kitName}/{noteName}_v{velocity}.wav` (例: `public/bass/upright/e2_v90.wav`)
- キットフォルダ名は `bass-patterns.generated.json` の `kits` で定義 (style → kit フォルダ)。複数スタイルが同じキットを共有可能
- サンプルマップは `bass-patterns.generated.json` の `samples` で定義 (style → { pitch: velocities[] })
- ベロシティレイヤーは自由。再生時は `findNearestVelocity()` で最寄り WAV を自動選択
- voice stealing: ベースは単音楽器 → 前ノートの duration 終了時 (または次の音の発音時の早い方) に停止 + リリースサンプル再生。duration 終了〜次の音発音の余白にリリースノイズが自然に挿入される
- **リリースサンプル**: `{noteName}_v{velocity}_rel.wav` — 非レガート時、前ノートの duration 終了タイミングで自動再生。リリース専用 Sampler (メインと同一設定)。WAV なしならハードカットにフォールバック。stop() 時はリリースも含め全停止
- **レガートサンプル**: 確率ベース判定 (`legatoProbability`, デフォルト11%, EZBass分析に基づく) + 方向判定。interval=0 (同音) は除外
  - **プリングオフ** (interval < 0): `{noteName}_p80.wav` — プリングオフ専用 Sampler (legatoByStyle)。WAV なしならピチカートにフォールバック。リリース: `_p80_rel.wav`
  - **ハンマリングオン** (interval > 0): `{noteName}_h80.wav` — ハンマリングオン専用 Sampler (hammerOnByStyle)。WAV なしならピチカートにフォールバック。リリース: `_h80_rel.wav`
- letRing: コード遷移時に未来ノートのみキャンセル、再生中の音は自然減衰
- WAV なしなら SoundFont (`acoustic_bass`) にフォールバック (デフォルト動作)
- **ベースパターン JSON** (`public/bass-patterns.generated.json`): `patterns` (スタイル別度数ベースパターンDB)、`weights`、`samples`、`kits`
- **ベース設定 JSON** (`public/bass-config.json`): `kitGains` (キット別ゲイン倍率)、`customWAV` (detune/decayTime/volume/octaveShift/releaseFadeMs(型定義のみ・未使用)/legatoMaxInterval/legatoProbability)、`prng` (シード)、`defaultDuration` (デフォルト0.86拍)、`velocityHumanize` (base/range)、`tripletGrace` (probability/velocity/beatOffset)、`patterns.swing` (SwingBassParams: テンプレート・コンター・アプローチ重み) — ウォーキングベース生成+再生チューニング
...
- POS_COLORS: `['#E74C3C', '#E67E22', '#E8336F', '#27AE60', '#6EAC00', '#8E44AD', '#16A085', '#2980B9']`

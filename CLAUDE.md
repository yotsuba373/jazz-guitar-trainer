...
│       ├── phraseGenerator.test.ts  — 39 tests (buildNotePool、getApproachNotes、構造不変条件、メタデータ、startHint連結、スケール互換フィルタ、chainFromStepフィルタ、リック連結、resolveLick品質ゲート)
...
**テスト**: 39テスト (buildNotePool + getApproachNotes + 構造不変条件 + メタデータ + startHint連結 + スケール互換フィルタ + chainFromStepフィルタ + リック連結 + resolveLick品質ゲート) + 1テスト (品質監査プレースホルダー) + 29テスト (分析・メタデータ・ナラティブ)

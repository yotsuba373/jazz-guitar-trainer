...
│   ├── configLoader.ts              — 汎用 JSON コンフィグローダー factory (deepMerge, createConfigLoader) + 全設定型定義 (DrumConfig/CompConfig/BassConfig(kitGains/customWAV(detune/decayTime/volume/releaseFadeMs/legatoMaxInterval/legatoProbability)/prng/defaultDuration/velocityHumanize/tripletGrace/patterns.swing(SwingBassParams)/altOctaveProb/styleOverrides(SwingStyleOverrides: altOctaveProb))/PianoConfig/AudioConfig)

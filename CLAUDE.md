...
│       ├── phraseGenerator.test.ts  — 80 tests (構造不変条件、ビバップ統計検証、メタデータ記録、startHint連結、骨格近接、ゴールVL保持、selectLick rootSemitone変換、クロスキー連結、スケール互換フィルタ、resolveLick品質ゲート)
│       ├── phraseQualityAudit.test.ts — 414 tests (17条件×23品質アサーション + 3進行コンテキスト + 5品質プロファイル)
...
**2パス生成**: リックライブラリ優先 → スコアリングフォールバック。可変リズム (4分/3連/8分/16分) 対応。

**アルゴリズム概要 (2パス生成)**:
1. **品質プロファイル解決**: `mode.chordQuality` → `PARKER_PROFILES` でインターバル重み・アプローチ分布・コンター重み・ピッチクラス選好を取得
2. `buildNotePool()`: ポジション内の全スケール音 + クロマチックアプローチ音を収集
3. インスタンススコープ (`activePool`): 第1インスタンスの fretMin-1 ~ fretMax+1 に制限（音域コンパクト化）
4. ゴール音決定: `goalNoteOverride` (ユーザー指定) → 進行モード次コード3rd → 通常モード強進行マッピング
5. **リック駆動パス (Pass 1)**: `selectLick()` で品質/拍数/ゴール互換リック選択 → `resolveLick()` で指板解決 (最大3リトライ)
6. 成功 → `PhraseNote[]` (duration, beatStart付き) 直接返却。ゴール未達時はコネクタ音追加
7. **全失敗 → スコアリングパス (Pass 2)** にフォールバック:
   - **ハーモニック・スケルトン**: `planCtSkeleton()` でアルペジオパターン事前決定
   - **デジタルパターン試行**: 42%確率、1フレーズ1回
   - 逐次生成: 候補スコアリング (インターバル分布, 輪郭一致, 方向転換, ゴール近接, 骨格追従, アルペジオ断片)
   - アプローチパターン挿入、ポスト処理

**リックライブラリ** (`public/data/lick_library.json`, ~3.8MB):
- WJD 186ソロ + Omnibook 50ソロから抽出、~20,000リック (品質別: dom7/min7/maj7/min7b5/dim7)
- `loadLickLibrary()`: fetch + キャッシュ、App初回マウント時プリロード
- `selectLick()`: 品質フィルタ → durationBeats制約 → **modeSemiスケール互換フィルタ** (uniqueOut≥2→除外, outCount×-15ペナルティ) → **CHARACTERISTIC_TONES特性音ボーナス** (一致×+20) → ゴール互換/startHint近接/コンター親和/充填率スコアリング → top10重み付きランダム
...
- **骨格拍間近接制御**: `resolveSkeletonBeat()` に prevBeat パラメータ追加。前骨格拍からの距離 >10st で 85%減衰、>7st で 60%減衰 → オクターブ跳躍を抑制

**デジタルパターン (Digital Patterns)**:
- 4音パターン17個: 上行4, 下行7 (3-R-7-5, 5-3-R-2, 3-2-R-7 追加), スカラー2, ハイブリッド6 (R-3-5-step-up, 7-5-step-up-3, 5-6-7-R 追加) の4カテゴリ
- `tryDigitalPattern()`: 42%発火率、1フレーズ1回、強拍CT一致+到達可能性チェック
- `pickByDirection()`: 方向に応じた最寄りインスタンス選択
- `findScaleDegreeNeighbor()`: スケール度数オフセットでの隣接音検索

**デジタルパターン**: 33パターン (上行4+下行7+スカラー2+ハイブリッド6 = 4音パターン19個 + 5音パターン10個 + 6音パターン4個)
...
**スコアリング重み**:
- 順次進行 (1-2半音): 60, 3度 (3-4半音): 18, 4度 (5半音): 14, 5度+: 5 ← **品質別倍率適用** (computeIntervalWeights)
- **ピッチクラス選好**: Parker品質別使用頻度偏差 × 1.5 (±12程度)
- 弦距離: -15/弦, 大跳躍: -25 (≥6半音), -35 (≥8半音)
- 方向転換ボーナス: +15, 同方向3+連続: -25
- **ガイドトーン階層**: 強拍3rd/7th +12, beat1/3追加 +6, エクステンション-10, エンクロージャー3rd/7thターゲット +12
- **ビバップ経過音**: Mixolydian/Dorian/Ionian のビバップスケール音 +20×品質別倍率 (弱拍+スカラー時)
- **弱拍スカラー優先**: 弱拍で非CT・非アプローチの順次進行 +8
...
- **3度ラン継続**: +22 (同方向3度が2連続以上 = R→3→5→7スタイルのアルペジオラン)
- スカラーラン継続: +14
- 停滞ペナルティ: -55 (≤2st), -35 (≤3st)
- goalProximityScore: キャップ +40, 半音解決ボーナス: +15
- contourScore: max 22
...
**フレーズ分析**: `phraseAnalysis.ts` — analyzePhrase(), 度数/インターバル/機能ラベル (全日本語), 生成メタデータ (スケルトン/デジタルパターン/ゴール理由/ビバップ/エクステンション), PhraseAnalysisPanel (折りたたみUI, 全ラベル日本語, ツールチップ説明付き, 骨格拍=背景ハイライト)

**UI**: OptionBar「フレーズ」チェックボックス → PhraseControls (ソース/アプローチ/Generate/◀▶履歴)
有効条件: `selPosIds.length === 1 && !overlay && !is8Note`

**テスト**: 80テスト (構造不変条件 + N=100 ビバップ統計検証 + メタデータ記録 + startHint連結 + 骨格近接 + ゴールVL保持 + selectLick rootSemitone変換 + クロスキー連結 + スケール互換フィルタ + resolveLick品質ゲート) + 409テスト (品質監査: 17条件×23アサーション + 3進行コンテキスト) + 33テスト (分析・メタデータ・ナラティブ)
...
- フレーズジェネレーター: 2パス生成 (リック駆動優先+スコアリングフォールバック)、リックライブラリ (~20,000リック、WJD+Omnibook)、可変リズム (4分/3連/8分/16分)、拍数選択 (2/3/4拍)、ゴールノート選択 (指板クリック)、ハーモニック・スケルトン+デジタルパターン33個+ガイドトーン階層+エクステンション音+ビバップスケール+コード間VL+モチーフ記憶、SVG曲線表示、アプローチパターン、品質監査・統計検証済、単体再生 (▶ Play)
- フレーズ分析: analyzePhrase(), PhraseAnalysisPanel (折りたたみUI、度数/インターバル/機能ラベル、生成メタデータ可視化、ナラティブ、ピアノロールSVG)

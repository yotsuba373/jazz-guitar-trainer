...
│   ├── lickEngine.ts                — absolutePitch(), buildNotePool(), loadLickDB(), transposeLick(), mapLickToFretboard(), lickToGeneratedPhrase(), inferModeFromLick(), inferModeCandidates(), findBestPositionForLick(), selectBestInstance(), buildLickContext()
...
リック選択も `ChordSlot.lickId` + `lickHighOctave` に保存され、コード切替時に自動復元。進行再生時は保存リックを自動再生。
...
- リック練習UI (練習モード): ChordChart直下の折りたたみパネル (LickPanel) にコード品質に合うリック一覧表示、安定ID(署名ハッシュ)+SVGコンター+音数/拍数+開始・終了度数&実音名+ソース名+モード候補(最大3, MODE_COLORSカラー)、テキスト検索(モード名・度数も対象)、選択→指板表示+自動再生、モード/ポジション自動推定、分析パネル対応、**リック選択をChordSlotに永続化** (lickId+lickHighOctave→コード切替時復元+進行再生時自動再生)。ルールベースフレーズ生成は削除済み (リック練習に一本化)
...
1. ii-V リック対応 (2コードにまたがるリック選択)
2. 密度フェーダーUI (noteCount/beats でフィルタ)
3. アーティスト別フィルタ

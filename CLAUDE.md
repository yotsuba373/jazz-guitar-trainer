...
│       ├── phraseGenerator.test.ts  — 56 tests (構造不変条件、ビバップ統計検証)
│       ├── phraseQualityAudit.test.ts — 136 tests (8条件×17品質アサーション)
...
4. **CT骨格事前計画**: `planCtSkeleton()` で beat 1,3,5,8 の強拍CTを contour curve に基づき事前決定
5. 逐次生成 (beats 2-7): 候補スコアリング (インターバル分布, 輪郭一致, 方向転換, ゴール近接, **骨格追従**)
6. アプローチパターン挿入: 弱拍で tryApproachCommitment() → 複数音一括確定
7. ポスト処理: 同音反復修正、beat 8 距離ガード (>5st でフォールバック)

**CT骨格 (CtSkeleton)**:
- `planCtSkeleton()`: contour curve から beat 1,3,5,8 の目標ピッチを算出、各強拍に異なるCT名を配置
- `pickCtNearPitch()`: 目標ピッチに近く既出CT名と異なる候補を重み付け選択
- フレーズ全体にR→3→5→7等の方向性あるアルペジオ骨格を付与 (パーカー的)

**アプローチタイプ**: Single↓/↑ (1音), Enclosure (2音), Parker Enclosure (3音), b9 Arpeggio (4音, Dom7のみ)

**スコアリング重み**:
- 順次進行 (1-2半音): 60, 3度 (3-4半音): 25, 4度 (5半音): 10, 5度+: 5
- 弦距離: -15/弦, 大跳躍: -20 (≥6半音), -30 (≥8半音)
- 方向転換ボーナス: +15, 同方向3+連続: -25
- **骨格追従**: +40 (CT名一致), +15 (同インスタンス近接)
- 往復ペナルティ: -80 (2-back), -40 (3-back)
- CTアウトライン進行: +12 (隣接CT), +6 (スキップ)
- パッシングトーン品質: +10 (骨格ピッチに向かう動き)
- 停滞ペナルティ: -30 (4音が3st以内)
- contourScore: max 30, 半音解決ボーナス: +15
...
**テスト**: 56テスト (構造不変条件 + N=100 ビバップ統計検証) + 136テスト (品質監査: 8条件×17アサーション) + 21テスト (分析)
...
- フレーズジェネレーター: ビバップライン自動生成 (CT骨格事前計画)、SVG曲線表示、アプローチパターン、品質監査・統計検証済

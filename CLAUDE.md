...
│       ├── phraseGenerator.test.ts  — 33 tests (buildNotePool、getApproachNotes、構造不変条件、メタデータ、startHint連結、スケール互換フィルタ、resolveLick品質ゲート)
...
5. **リック選択+解決ループ (5リトライ)**: `selectLick()` → `resolveLick()` → 成功なら即 return
6. コネクタ音追加 (ゴール未達時)、モチーフ抽出
7. 全失敗 → `return null`
...
**テスト**: 33テスト (buildNotePool + getApproachNotes + 構造不変条件 + メタデータ + startHint連結 + スケール互換フィルタ + resolveLick品質ゲート) + 1テスト (品質監査プレースホルダー) + 33テスト (分析・メタデータ・ナラティブ)
...
- フレーズジェネレーター: リック検索型アーキテクチャ (selectLick→resolveLick、5リトライ)、リックライブラリ (~20,000リック、WJD+Omnibook)、可変リズム (4分/3連/8分/16分)、拍数選択 (2/3/4拍)、ゴールノート選択 (指板クリック)、コード間VL+モチーフ記憶、SVG曲線表示、単体再生 (▶ Play)

...
│   ├── configLoader.ts              — 汎用 JSON コンフィグローダー factory (deepMerge, createConfigLoader) + 全設定型定義 (DrumConfig/CompConfig/BassConfig(kitGains/customWAV/prng/defaultDuration/velocityHumanize/tripletGrace/patterns.swing(SwingBassParams)/styleOverrides(SwingStyleOverrides))/PianoConfig/AudioConfig)
...
│       └── configLoader.test.ts   — 13 tests (deepMerge, 各getXxxConfig未ロードデフォルト値)
├── hooks/
...
- voice stealing: ベースは単音楽器 → 新ノート発音時に前ノートを停止予約
...
- **ベース設定 JSON** (`public/bass-config.json`): `kitGains` (キット別ゲイン倍率)、`customWAV` (detune/decayTime/volume)、`prng` (シード)、`defaultDuration` (デフォルト0.86拍)、`velocityHumanize` (base/range)、`tripletGrace` (probability/velocity/beatOffset)、`patterns.swing` (SwingBassParams: テンプレート・コンター・アプローチ重み) — ウォーキングベース生成+再生チューニング

...
- voice stealing: ベースは単音楽器 → 新ノート発音時に前ノートを停止予約 + リリースサンプル再生
- **リリースサンプル**: `{noteName}_v{velocity}_rel.wav` — voice stealing 時に自動再生。リリース専用 Sampler (メインと同一設定)。WAV なしならハードカットにフォールバック。stop() 時はリリースも含め全停止

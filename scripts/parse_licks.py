...
2つの入力モード:
  A) マルチトラック MIDI (推奨):
     python scripts/parse_licks.py scripts/data/export_licks.mid
     → トラック名がファイル名規約に従う (例: "cannonball_dom7_b1")

  B) 個別ファイル / ディレクトリ:
     python scripts/parse_licks.py scripts/data/midi/licks/parker_dom7_b1.mid
...
出力:
  scripts/data/licks.generated.json — タイプ別統合リックDB
  → public/licks.generated.json にコピー
"""
import os
import sys
import json
...
def save_and_report(db, total_new):
    ...
    output_path = os.path.join(script_dir, "data", "licks.generated.json")
    public_path = os.path.join(script_dir, "..", "public", "licks.generated.json")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(__doc__)
        print("使用例:")
        print("  python scripts/parse_licks.py scripts/data/export_licks.mid              # マルチトラック (推奨)")
        print("  python scripts/parse_licks.py scripts/data/midi/licks/parker_dom7_b1.mid            # 個別ファイル")
        print("  python scripts/parse_licks.py scripts/data/midi/licks/                         # ディレクトリ内全ファイル")
        sys.exit(1)

...
2つの入力モード:
  A) マルチトラック MIDI (推奨):
     python scripts/parse_licks.py scripts/output/export_licks.mid
     → トラック名がファイル名規約に従う (例: "cannonball_dom7_b1")

  B) 個別ファイル / ディレクトリ:
     python scripts/parse_licks.py scripts/output/midi/licks/parker_dom7_b1.mid
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(__doc__)
        print("使用例:")
        print("  python scripts/parse_licks.py scripts/output/export_licks.mid              # マルチトラック (推奨)")
        print("  python scripts/parse_licks.py scripts/output/midi/licks/parker_dom7_b1.mid            # 個別ファイル")
        print("  python scripts/parse_licks.py scripts/output/midi/licks/                         # ディレクトリ内全ファイル")
        sys.exit(1)
